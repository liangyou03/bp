...
class UnlabeledECGPPGNPZ(Dataset):
    ...
    def __getitem__(self, idx: int):
        ...
        ecg = torch.from_numpy(ecg_up[None, :].copy())  # (1, 3630)
        ppg = torch.from_numpy(ppg_raw[None, :].copy()) # (1, 3630)
...
# ---------------------------
@torch.no_grad()
def retrieval_top1(z_a, z_b):
    logits = z_a @ z_b.t()
    pred = logits.argmax(dim=1)
    gt = torch.arange(z_a.size(0), device=z_a.device)
