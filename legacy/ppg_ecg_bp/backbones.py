...
Minimal changes:
- AgeModel supports multi-target output via target_dim
- Keep load_from_pretrain behavior unchanged
- Backward compatible: if target_dim == 1, forward returns shape (B,)
"""
...
class BasicBlockPPG(nn.Module):
    def __init__(
        ...
        r = 2
        self.se1 = nn.Linear(out_channels, out_channels // r)
        self.se2 = nn.Linear(out_channels // r, out_channels)

        self.is_first_block = is_first_block
        if self.downsample:
            self.pool = MyMaxPool1dPadSame(kernel_size=stride)
...
class Net1D_PPG(nn.Module):
    def __init__(self):
        ...
        for i, (oc, mb) in enumerate(zip(filt, blocks)):
            stages.append(StagePPG(in_ch, oc, 1, 3, 2, 16, i, mb, True, True))
            in_ch = oc
...
class MyConv1dPadSame_ECG(nn.Module):
    ...
    def forward(self, x):
        L = x.shape[-1]
        out = (L + self.stride - 1) // self.stride
...
class BasicBlockECG(nn.Module):
    def __init__(
        ...
        r = 2
        self.se1 = nn.Linear(out_ch, out_ch // r)
        self.se2 = nn.Linear(out_ch // r, out_ch)

        self.downsample = downsample
        if self.downsample:
            self.pool = MyMaxPool1dPadSame_ECG(stride)
    ...
    def __init__(self, in_ch, out_ch, ratio, ksize, stride, groups_width, i_stage, m_blocks, use_bn=False, use_do=False):
        ...
        for i in range(m_blocks):
            ...
            blk = BasicBlockECG(
                ...
                out_ch // groups_width,
                down,
                is_first_block=(i_stage == 0 and i == 0),
                use_bn=use_bn,
                use_do=use_do,
            )
            blocks.append(blk)
...
class ECGBackbone1L(nn.Module):
    def __init__(self):
        ...
        for i, (oc, mb) in enumerate(zip(filt, m)):
            stages.append(StageECG(in_ch, oc, 1, 16, 2, 16, i, mb, False, False))
            in_ch = oc
        self.stages = nn.ModuleList(stages)
...
class AgeModel(nn.Module):
    """
    Backward compatible:
    - target_dim == 1 -> returns (B,)
    - target_dim > 1  -> returns (B, target_dim)
    """
    ...
    def load_from_pretrain(self, ckpt_path: str, device):
        ...
        print(
            f"[load pretrain] from {ckpt_path}\n"
    ...
    def forward(self, ecg, ppg):
        ...
        out = self.head(z)  # (B, target_dim)

        # Keep backward compatibility with age script expecting (B,)
        if out.shape[-1] == 1:
