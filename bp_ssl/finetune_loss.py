# finetune_loss.py
# -------------------------------------------------------------
# Loss utilities for age regression fine-tuning
# Includes:
# 1) DistributionAlignmentLoss (KDE prior + batch soft histogram alignment)
# 2) Inverse-frequency weighted MSE (age imbalance handling)
...
def build_kde_prior(
    ...
    """
    KDE on discrete bin centers to estimate age prior distribution.
    Returns:
      bin_centers: (Bin,)
      prior_probs: (Bin,) sum=1
    """
    train_ages = np.asarray(train_ages, dtype=np.float64)
    bin_centers = np.arange(y_min, y_max + 1e-6, bin_width, dtype=np.float64)

    diffs = (bin_centers[None, :] - train_ages[:, None]) / (sigma + eps)
...
class DistributionAlignmentLoss(nn.Module):
    ...
    - Convert batch predictions into a soft histogram q (Gaussian membership on bins)
    - Match q to training prior p with optional inverse-frequency weights
    """
    def __init__(
        self,
    ...
    def forward(self, y_pred: torch.Tensor) -> torch.Tensor:
        """
        y_pred: (B,) predicted ages (recommended clamped to [y_min, y_max])
        """
        y = y_pred.reshape(-1, 1)                 # (B,1)
        centers = self.bin_centers.reshape(1, -1) # (1,Bin)
        d2 = (y - centers) ** 2                   # (B,Bin)

        weights = torch.exp(-0.5 * d2 / (self.sigma2 + self.eps))
        weights = weights / (weights.sum(dim=1, keepdim=True) + self.eps)

        q = weights.sum(dim=0)  # (Bin,)
        q = q / (q.sum() + self.eps)
...
# -----------------------------
def build_invfreq_weights_from_train_ages(
    train_ages: np.ndarray,
    y_min: float,
    y_max: float,
    bin_width: float = 1.0,
    ...
    """
    Build inverse-frequency weights by histogram (on integer bins).
    Returns:
      bin_centers: (Bin,)
      weights: (Bin,) weights for each age bin
    """
    train_ages = np.asarray(train_ages, dtype=np.float64)
    bin_centers = np.arange(y_min, y_max + 1e-6, bin_width, dtype=np.float64)

    # Map each train age to nearest integer bin
    idx = np.clip(np.rint(train_ages).astype(np.int64), int(y_min), int(y_max)) - int(y_min)
...
class InverseFreqWeightedMSELoss(nn.Module):
    """
    Weighted MSE by inverse-frequency weights computed on age bins.
    """
    def __init__(
        self,
        bin_centers: np.ndarray,
...
class PearsonLoss(nn.Module):
    """
    Pearson correlation loss: 1 - corr(y_pred, y_true)
    """
...
def prepare_losses(
    ...
    loss_mode:
      ...
      - "mse*pearson"
    Returns dict with:
      { "loss_mode", "y_min","y_max", ...modules... }
    """
    assert loss_mode in ["mse", "mse+dist", "wmse", "mse*pearson"]

    train_ages = np.asarray(train_ages, dtype=np.float32)
    if y_min is None:
        y_min = float(np.floor(train_ages.min()))
    if y_max is None:
        y_max = float(np.ceil(train_ages.max()))
    ...
    if loss_mode == "mse":
        ...
        centers, weights = build_invfreq_weights_from_train_ages(
            train_ages=train_ages,
            y_min=y_min,
            y_max=y_max,
