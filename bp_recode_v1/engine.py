...
from utils import (apply_constraint, mae_np, rmse_np,
                   pearson_r_safe_np, r2_np)

try:
    from dist_loss import DistributionAlignmentLoss  # optional dependency
    _HAS_DIST = True
...
def evaluate_with_ids(model, loader, device, modality,
    ...
    if constrain != "none":
        yhat = np.clip(yhat, y_min, y_max)

    mae = mae_np(y, yhat)
    rmse = rmse_np(y, yhat)
    r = pearson_r_safe_np(y, yhat)
    r2 = r2_np(y, yhat)
...
def train_one_epoch(model, loader, optimizer, scaler, device, modality, loss_type,
    ...
    """训练一个epoch，返回(avg_loss, train_mae)"""
    model.train()
    total = 0
    total_loss = 0.0
    total_ae = 0.0
    ...
    for batch in pbar:
        ...
        total_loss += float(loss.item()) * bs
        with torch.no_grad():
            total_ae += float(torch.abs(yhat - y).sum().item())
        train_mae = total_ae / total
        pbar.set_postfix(loss=f"{total_loss / total:.4f}", MAE=f"{train_mae:.2f}")

    pbar.close()
    return total_loss / total, total_ae / total
