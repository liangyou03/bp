...
class AgeModel(nn.Module):
    ...
    def load_from_pretrain(self, ckpt_path: str, device):
        """
        Load JOINT ECG–PPG pretrained checkpoint (Age-style).
        This expects a single checkpoint containing both encoders.
        """
        ...
        loadable = {
            ...
            if k in own and (k.startswith("ecg_enc.") or k.startswith("ppg_enc."))
        }

        msg = self.load_state_dict({**own, **loadable}, strict=False)

        print(
            f"[load joint pretrain] {ckpt_path}\n"
            f"  loaded={len(loadable)} params\n"
    ...
    def forward(self, ecg, ppg):
        ...
        if self.modality in ["ecg", "both"]:
            zs.append(self.ecg_enc(ecg))   # (B, 512)

        if self.modality in ["ppg", "both"]:
            zs.append(self.ppg_enc(ppg))   # (B, 512)
